...
const initBigTypeSelectList=async()=>{
  const res=await axios.get("http://localhost:9091/bigType/listAll");
...
const rules=ref({
  name: [
    { required: true, message: '请输入食品小类名称'}
  ],
  remark: [
    { required: true, message: '请输入食品小类描述'}
  ]
})
...
const initFormData=async(id)=>{
  const res = await axios.get("http://localhost:9091/smallType/"+id);
  ...
  formRef.value.validate(async(valid)=>{
    if(valid){
      if(form.value.bigType.id==""){
        ElMessage.error("请选择食品大类");
        return;
      }
      let result=await axios.post("http://localhost:9091/smallType/save",form.value);
