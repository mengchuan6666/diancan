  ...
  >
    ...
    >
      ...
      <el-upload
          ...
          :action="getServerUrl()+'/productSwiperImage/uploadImage'"
...
const initProductSwiperImageList = async()=>{
  const res=await axios.get("/productSwiperImage/list/"+form.value.productId)
  ...
  const handleConfirm=async()=>{
    formRef.value.validate(async(valid)=>{
      if(valid){
        let result=await axios.post("/productSwiperImage/add",form.value);
...
const handleDelete=(id,status)=>{
  ElMessageBox.confirm(
      ...
      .then(async () => {
        let res = await axios.get('/productSwiperImage/delete/'+id)
        if(res.data.code==0){
          ElMessage({
            type:'success',
            message:'删除成功',
